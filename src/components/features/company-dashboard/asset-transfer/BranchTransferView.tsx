...
export function BranchTransferView({ searchQuery = '' }: { searchQuery?: string }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!cancelled) setData(response.data.filter((t) => t.transferType === 'branch'));
      } catch {
        if (!cancelled) setError('Failed to load branch transfers.');
      } finally {
  ...
  const handleCreate = () => {
    router.push(`${ROUTES.COMPANY_DASHBOARD_ASSET_TRANSFER}/branch/add`);
  };
  ...
  const filteredData = data.filter(row => 
    ...
    (row.toBranchName && row.toBranchName.toLowerCase().includes(searchQuery.toLowerCase()))
  );
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead className="bg-[#DCEBFE] sticky top-0 z-10">
              <tr>
                ...
                <th className="py-4 px-6 text-[13px] font-bold text-gray-800 text-center">Branch Name</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="py-10 text-center text-[13.5px] font-medium text-gray-500">
                    No branch transfers found matching your search.
                  </td>
                </tr>
              ) : (
                filteredData.map((row) => (
                  <tr key={row.id} className="border-b border-[#E9EFF6] last:border-0 hover:bg-gray-50/50">
                    ...
                    <td className="py-4 px-6 text-[13.5px] font-medium text-gray-700 text-center">
                      {row.toDepartmentName || row.fromDepartmentName || '-'}
                    </td>
                    <td className="py-4 px-6 text-[13.5px] font-medium text-gray-700 text-center">
                      {row.toBranchName || '-'}
                    </td>
...
export default BranchTransferView;
